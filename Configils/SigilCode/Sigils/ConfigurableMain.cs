...
namespace JLPlugin.SigilCode
{
    public class ConfigurableMain : ConfigurableBase, IOnBellRung, IOnOtherCardAddedToHand
    {
        ...
        public override IEnumerator OnOtherCardResolve(PlayableCard otherCard)
        {
            foreach (AbilityBehaviourData behaviourData in abilityData.abilityBehaviour)
            {
                if (otherCard.Slot.opposingSlot.Card == base.Card || behaviourData.trigger.activatesForCardsWithCondition != null)
                {
        ...
        {
            bool hasActivateTrigger = abilityData.abilityBehaviour.Select(x => x.trigger.triggerType).Contains("OnActivate");
            AbilityBehaviourData behaviourData = abilityData.abilityBehaviour.Where(x => x.trigger.triggerType == "OnActivate").ToList()[0];
            SigilData.UpdateVariables(behaviourData, base.Card);
            string condition = behaviourData.trigger.activatesForCardsWithCondition;

            return hasActivateTrigger && (SigilData.ConvertArgument(condition, behaviourData) ?? "false") == "true";
        }
